...
namespace DigiMa
{
    public partial class EditCanvasAreaPromoTwo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request["Maint"] != null)
                {
                    ...
                    startDate = SessionData.Config.SAppStartDT;
                    endDate = SessionData.Config.AppExpiryDT;
                    if (!string.IsNullOrEmpty(startDate))
                    {
                        DateTime start = DateTime.Parse(startDate);
                        datepickerStart.Value = start.ToString("dd/MM/yyyy");
                    }
        ...
        protected void btnStep1Complete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //hdnWidthStatus.Value = txtCanvasHeight.Text.Trim();
                //hdnHeightStatus.Value = txtCanvasWidth.Text.Trim();
        ...
        protected void btnStep2Complete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (filePromo2.HasFile)
                {
                    ...
                    {
                        if (filePromo2.FileBytes.Length < 1048576)
                        {
                            ...
                            if (chkPromo2.Checked)
                            {
                                ...
                                {
                                    SaveImageFile(sourceImage, Server.MapPath(pathToCreate + "/Head" + filenamePromo2), Convert.ToInt32(SessionData.Product.CanvasHeight), Convert.ToInt32(SessionData.Product.CanvasWidth));
                                }
                                hdnFooter.Value = filenamePromo2;
                                SessionData.Product.ProductLogo = filenamePromo2;
                            }
                            else
                            {
