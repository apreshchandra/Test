...
namespace DigiMa
{
    public partial class CanvasAreaPromoTwo : System.Web.UI.Page
    {
        ...
        private const string TEMPLATE_PAGE = "EditCanvasAreaPromoTwo.aspx";
        ...
        protected void btnStep2Complete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //Body Image
                if (filePromo2.HasFile)
                {
                    if (filePromo2.PostedFile.ContentType == "image/jpeg" || filePromo2.PostedFile.ContentType == "image/png" || filePromo2.PostedFile.ContentType == "image/jpg" || filePromo2.PostedFile.ContentType == "image/pjpeg")
                    {
                        if (filePromo2.FileBytes.Length < ONE_MB)
                        {
                            string filenamePromo2 = Path.GetFileName(filePromo2.FileName);
                            if (chkPromo2.Checked)
                            {
                                Bitmap sourceImage = new Bitmap(filePromo2.PostedFile.InputStream);
                                if (SessionData.Product.CanvasHeight == string.Empty)
                                {
                                    SaveImageFile(sourceImage, Server.MapPath(pathToCreate + "/Head" + filenamePromo2), Convert.ToInt32(SessionData.Product.CanvasWidth), 700);
                                }
                                else if (SessionData.Product.CanvasWidth == string.Empty)
                                {
                                    SaveImageFile(sourceImage, Server.MapPath(pathToCreate + "/Head" + filenamePromo2), 500, Convert.ToInt32(SessionData.Product.CanvasHeight));
                                }
                                else if (SessionData.Product.CanvasHeight == string.Empty && SessionData.Product.CanvasWidth == string.Empty)
                                {
                                    SaveImageFile(sourceImage, Server.MapPath(pathToCreate + "/Head" + filenamePromo2), 500, 700);
                                }
                                else
                                {
                                    SaveImageFile(sourceImage, Server.MapPath(pathToCreate + "/Head" + filenamePromo2), Convert.ToInt32(SessionData.Product.CanvasWidth), Convert.ToInt32(SessionData.Product.CanvasHeight));
                                }
                                hdnFooter.Value = filenamePromo2;
                            }
                            else
                            {
                                filePromo2.PostedFile.SaveAs(Server.MapPath("~/Images/" + SessionData.Product.DID + "/Head" + filenamePromo2));
                                hdnFooter.Value = filenamePromo2;
                            }
                ...
                if (imgBanner.HasFile)
                {
                    if (imgBanner.PostedFile.ContentType == "image/jpeg" || imgBanner.PostedFile.ContentType == "image/png" || imgBanner.PostedFile.ContentType == "image/jpg" || imgBanner.PostedFile.ContentType == "image/pjpeg")
                    {
                        if (imgBanner.FileBytes.Length < ONE_MB)
                        {
                            ...
                            {
                                SaveImageFile(sourceImage, Server.MapPath(pathToCreate + "/Banner" + imgBannerPromo2), Convert.ToInt32(SessionData.Product.CanvasWidth), Convert.ToInt32(SessionData.Product.CanvasHeight));
                            }
        ...
        protected void btnStep3Complete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (fbBiz.IsAppCreationAllowed(SessionData.Customer.CustomerID)) //check if user is allowed to create Promos etc under his current plan *****************************************************************
                {
                    ...
                    string HeaderBannerURLReplaced = tempData.Replace(" <a id=\"aHeadBanner\" href=\"\" target=\"_blank\">", "<a id=\"aHeadBanner\" href=" + txtBannerURL.Text + " target=\"_blank\" >"); // Replace Banner URL
                    ...
                    string BodyImageReplaced = HeaderBannerReplaced.Replace("<img id=\"imgHeader\" alt=\"\" src=\"\"", "<img id=\"imgHeader\" alt=\"\" src=\"Images/" + SessionData.Product.DID + "/" + "Head" + hdnFooter.Value + "\"");
                    ...
                    using (CanvasBizProcess canvasBiz = new CanvasBizProcess())
                    {
                        ...
                        canvasBiz.InsertPreviewHTML(BodyImageReplaced, SessionData.Customer.CustomerID, SessionData.Product.DID);
                    ...
                    if (!(SessionData.Config == null))
                    {
                        ...
                        SessionData.Config.SCampaignType = Promo;
                        if (fbBiz.SetNewConfigDetails(SessionData.Config, SessionData.Config.SCustomtTabName,Promo))
                        {
