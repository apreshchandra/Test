...
// *********************************SM NETSERV COPYRIGHT*********************************************
// * FILE HISTORY
// * ~~~~~~~~~~~~~
// * 
// * WRITTEN BY    : Your Name
// * WRITTEN DATE  : Year Month Date [YYYY MM DD]
// * DESCRIPTION   :
// * MODIFICATIONS :
// * 
// * 
// * 
// * Copyright SM NetServ Technologies Pvt Ltd 2009 - Offshore Software Development Company, India
// *************************************************************************************************
...
namespace DigiMa
{
    internal class DatabaseHelper : IDatabaseHandler, IDisposable
    {
        ...
        {
            InsertExecutionLogs(sCommandText);
            try
            {
                // CurrentActivityEvent("Filling Data.");
        ...
        public void InsertExecutionLogs(string sSPNameOrQuery)
        {
            try
            {
                if (bool.Parse(CommonUtility.AppSettings("DebugLog", "false")))
                {
                    Open();

                    using (DbCommand _oDBComm = _Connection.CreateCommand())
                    {
                        _oDBComm.CommandText = "sp_INSERTEXECUTIONLOGS";
                        _oDBComm.CommandType = CommandType.StoredProcedure;
                        _oDBComm.CommandTimeout = 0;
                        _oDBComm.Parameters.Clear();
                        oDatabase.DiscoverParameters(_oDBComm);

                        foreach (IDbDataParameter tmp2 in _oDBComm.Parameters)
                        {
                            if (tmp2.ParameterName.EndsWith("SPNameOrQuery"))
                            {
                                tmp2.Value = sSPNameOrQuery;
                            }
                            else if (tmp2.ParameterName.EndsWith("ConnectionID"))
                            {
                                tmp2.Value = ConnectionID.ToString();
                            }
                        }
                        oDatabase.ExecuteNonQuery(_oDBComm);
                    }
                }
            }
            catch (Exception)
